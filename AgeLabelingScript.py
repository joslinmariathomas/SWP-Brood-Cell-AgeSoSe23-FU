...
def get_label_counter(row_ind, col_ind, frame_ind, frames, thresh_back, thresh_ahead, use_for_transition = True):
    ...
    for i in range(thresh_back, thresh_ahead+1):
        if frame_ind+i < len(frames) \
            and (frames[frame_ind + i]['cells'][row_ind][col_ind]['pred_label'] != '(unknown)' or use_for_transition) \
...
def get_current_state(row_ind, col_ind, frame_ind, frames, most_common_current):
    i = 0
    ...
    # if there is no stage that would fit with the age then use the stage with the most labels
    i = 0
    while len(most_common_current) > i and current_state == '(empty)':
        current_state = most_common_current[i][0]
        i+=1
    ...
    for frame_ind in range(2067):
        ...
        for row_ind, cell_row in enumerate(cells):
            for col_ind, cell in enumerate(cell_row):
                """
                if frame_ind == 0 and frames[frame_ind]['cells'][row_ind][col_ind]['pred_label'] != '(empty)':
                    first_label = frames[frame_ind]['cells'][row_ind][col_ind]['pred_label']
                    print(f'not empty: {row_ind}, {col_ind} with label {first_label}')
                """
                ...
                # transition from not empty to empty
                    ...
                    cnt_current = get_label_counter(row_ind, col_ind, frame_ind, frames, thresh_back_current, thresh_ahead_current, use_for_transition=False)

                    # most common labels sorted by count
                    most_common_current = cnt_current.most_common()

                    # get most likely current state
                    current_state = get_current_state(row_ind, col_ind, frame_ind, frames, most_common_current)
                    ...
                    if current_state == '(has egg)':
                        thresh_ahead_start = 200
                        thresh_back_start = -170
                    if current_state == '(has young pupa)' or current_state == '(has old pupa)':
                        thresh_ahead_start = 200
                        thresh_back_start = -80
                    if current_state == '(has bee head)':
                        thresh_ahead_start = 80
                        thresh_back_start = -50
                    ...
                    match current_state:
                        case '(has egg)':
                            min_empty = np.round(cnt.total() * 0.7)
                            ...
                            actual_current_or_later = cnt[current_state] + cnt['(has larva)'] + cnt['(has young pupa)'] + cnt['(has old pupa)'] + cnt['(has bee head)'] + cnt['(unknown)']
                        case '(has larva)':
                            min_empty = np.round(cnt.total() * 0.75)
                            max_current_or_later = np.round(cnt.total() * 0.2)
                            actual_current_or_later = cnt[current_state] + cnt['(has young pupa)'] + cnt['(has old pupa)'] + cnt['(has bee head)'] + cnt['(unknown)']
                        case '(has young pupa)':
                            min_empty = np.round(cnt.total() * 0.8)
                            max_current_or_later = np.round(cnt.total() * 0.2)
                            actual_current_or_later = cnt[current_state] + cnt['(has old pupa)'] + cnt['(has bee head)'] + cnt['(unknown)']
                        case '(has old pupa)':
                            min_empty = np.round(cnt.total() * 0.8)
                            max_current_or_later = np.round(cnt.total() * 0.2)
                            # here we also use has young pupa (although it's an earlier stage) because both are frequently confused
                            actual_current_or_later = cnt[current_state] + cnt['(has young pupa)'] + cnt['(has bee head)'] + cnt['(unknown)']
                        case '(has bee head)':
                            min_empty = np.round(cnt.total() * 0.7)
                            max_current_or_later = np.round(cnt.total() * 0.3)
                            actual_current_or_later = cnt[current_state] + cnt['(unknown)']
                        case _:
                            wrong_trans = True
                            actual_current_or_later = -1

                    actual_empty = cnt['(empty)']
                    if row_ind == 1 and col_ind == 1 and frame_ind == 970:
                    ...
                    if wrong_trans:
                        age = time - new_egg_timestamps[row_ind][col_ind]
                        # if age > 23.5 days force transition
                        if age > 2030400:
                            age = 0
                            wrong_trans = False
                    else:
